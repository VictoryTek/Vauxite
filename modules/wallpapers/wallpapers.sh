...
wallpaper_destination="/usr/share/backgrounds/vauxite"
...
xml_modified_template="$wallpapers_module_dir"/vauxite-template.xml
xml_destination="/usr/share/gnome-background-properties"
gschema_override="$wallpapers_module_dir"/zz2-vauxite-wallpapers.gschema.override
gschema_override_test_dir="/tmp/vauxite-schema-test-wallpapers"
gschema_override_test="$gschema_override_test_dir"/zz2-vauxite-wallpapers.gschema.override
...
# Set name of the XML to vauxite-nameofthewallpaper.jpg.xml
...
echo "Writing XMLs for included wallpapers to appear in Gnome settings"
  for wallpaper in "${WALLPAPER[@]}"; do
      ...
      yq -i '.wallpapers.wallpaper.name = "vauxite-'"$wallpaper"'"' "$xml_modified_template"
      ...
      cp "$xml_modified_template" "$xml_destination"/vauxite-"$wallpaper".xml
...
# Set name of the XML to vauxite-wallpaper-bb-light.jpg_+_vauxite-wallpaper-bb-dark.jpg.xml
if [[ ${#WALLPAPER_LIGHT_DARK[@]} -gt 0 ]]; then
  ...
  for ((i=0; i<${#WALLPAPER_LIGHT_DARK[@]}; i+=2)); do
    ...
    cp "$xml_default_template" "$xml_modified_template"
    yq -i '.wallpapers.wallpaper.name = "vauxite-'"$wallpaper_light"_+_"$wallpaper_dark"'"' "$xml_modified_template"
    ...
    cp "$xml_modified_template" "$xml_destination"/vauxite-"$wallpaper_light"_+_"$wallpaper_dark".xml
    rm "$xml_modified_template"
  done
fi
...
echo "Writing XML for default wallpaper to appear in Gnome settings"
  for default_wallpaper in "${DEFAULT_WALLPAPER[@]}"; do
      cp "$xml_default_template" "$xml_modified_template"
      yq -i '.wallpapers.wallpaper.name = "vauxite-'"$default_wallpaper"'"' "$xml_modified_template"
      yq -i ".wallpapers.wallpaper.filename = \"$wallpaper_destination/$default_wallpaper\"" "$xml_modified_template"
      yq 'del(.wallpapers.wallpaper.filename-dark)' "$xml_modified_template" -i
      cp "$xml_modified_template" "$xml_destination"/vauxite-"$default_wallpaper".xml
      rm "$xml_modified_template"    
  done
fi
...
echo "Writing XML for default light+dark wallpaper to appear in Gnome settings"
  for default_wallpaper_light_dark in "${DEFAULT_WALLPAPER_LIGHT_DARK[@]}"; do
        cp "$xml_default_template" "$xml_modified_template"
        yq -i '.wallpapers.wallpaper.name = "vauxite-'"$default_wallpaper_light_dark"'"' "$xml_modified_template"
        ...
        cp "$xml_modified_template" "$xml_destination"/vauxite-"$default_wallpaper_light_dark".xml
        rm "$xml_modified_template"
  done
fi
...
for scaling_option in "${scaling_options[@]}"; do
    ...
    if [[ $scaling_all == "all" ]]; then
        ...
        for xml_file in "${WALLPAPER[@]}" "${DEFAULT_WALLPAPER[@]}" "${DEFAULT_WALLPAPER_LIGHT_DARK[@]}"; do
            yq -i '.wallpapers.wallpaper.options = "'"$scaling_option"'"' "$xml_destination"/vauxite-"$xml_file".xml
        done
        for ((i=0; i<${#WALLPAPER_LIGHT_DARK[@]}; i+=2)); do
            wallpaper_light="${WALLPAPER_LIGHT_DARK[i]}"
            wallpaper_dark="${WALLPAPER_LIGHT_DARK[i+1]}"
            yq -i '.wallpapers.wallpaper.options = "'"$scaling_option"'"' "$xml_destination"/vauxite-"$wallpaper_light"_+_"$wallpaper_dark".xml
        done    
    fi
done
...
for scaling_option in "${scaling_options[@]}"; do
    ...
    if [[ -n $scaling_specific ]]; then
        for scaling_per_wallpaper in $scaling_specific; do
            yq -i '.wallpapers.wallpaper.options = "'"$scaling_option"'"' "$xml_destination"/vauxite-"$scaling_per_wallpaper".xml
        done
    fi
done
